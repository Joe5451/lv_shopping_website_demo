    ...
    <form action="{{ route('admin.product_update', $product->id) }}" method="post" enctype="multipart/form-data" class="admin_form max-w-screen-sm">
        ...
        <div class="form_group">
            <div class="flex items-start">
                <label class="form_label">商品圖片</label>
                <button type="button" class="admin_plus_btn ml-2" id="product_secondary_img_add_btn">
                    <i class="fas fa-plus-circle"></i>
                </button>
            </div>

            <div id="product_secondary_img_container" class="flex flex-wrap" style="margin-right:-12px;margin-left:-12px;">
                @foreach ($product->product_imgs as $product_img)
                    <?php $img_src = ($product_img->src != '') ? env('IMG_URL') . $product_img->src : ''; ?>

                    <div class="product_secondary_img_wrap mb-10 w-full md:w-1/2 px-3">
                        <input type="hidden" name="secondary_img_ids[]" value="{{ $product_img->id }}">
                        <input type="hidden" name="secondary_img_indexes[]" value="{{ $loop->index }}">
                        <input type="hidden" name="secondary_img_change[]" value="false">
                        <input type="file" name="secondary_img_{{ $loop->index }}" class="dropify_img" data-max-file-size="1M" data-default-file="{{ $img_src }}" data-show-remove="false" />

                        <div class="my-2 flex items-center">
                            <label class="shrink-0 text-slate-500">順序：</label>
                            <input type="number" name="secondary_img_sequences[]" value="{{ $product_img->sequence }}" class="w-16 border border-slate-400 py-1 px-2 focus:border-cyan-400 mr-4" required>
                            <div onclick="deleteImg(this)" class="ml-auto bg-red-500 hover:bg-red-600 px-3 py-1 text-white rounded cursor-pointer">移除圖片</div>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <script>
                let drEvent_secondary = $('.dropify_img').dropify({
                    messages: {
                        'default': '將文件拖放到此處或單擊',
                        'replace': '拖放或點擊替換',
                        'remove':  '刪除',
                ...
                $('#product_secondary_img_add_btn').click(function() {
                    $('#product_secondary_img_container').prepend(`
                        <div class="product_secondary_img_wrap mb-10 w-full md:w-1/2 px-3">
                            <input type="hidden" name="secondary_img_ids[]" value="new">
                            <input type="hidden" name="secondary_img_indexes[]" value="${current_secondary_img_index}">
                            <input type="hidden" name="secondary_img_change[]" value="true">
                            <input type="file" name="secondary_img_${current_secondary_img_index}" class="dropify_img" data-max-file-size="1M" data-show-remove="false" required />

                            <div class="my-2 flex items-center">
                                <label class="shrink-0 text-slate-500">順序：</label>
                                <input type="number" name="secondary_img_sequences[]" value="0" class="w-16 border border-slate-400 py-1 px-2 focus:border-cyan-400 mr-4" required>
                                <div onclick="deleteImg(this)" class="ml-auto bg-red-500 hover:bg-red-600 px-3 py-1 text-white rounded cursor-pointer">移除圖片</div>
                            </div>
                        </div>
                    `);
                    ...
                    current_secondary_img_index++;
                });

                function deleteImg(current_element) {
                    var element = $(current_element);
                    element.parents('.product_secondary_img_wrap').remove();
                }
            </script>
        </div>
